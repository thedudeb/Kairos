...
Runs as a separate local process from the same `backend/` source:
...
def _redis_settings() -> RedisSettings:
    ...
    # certificate that may not pass strict verification in some local setups.
...
class WorkerSettings:
    ...
    job_timeout = 60   # local PDF extraction and deterministic scoring
